...
class ClassifierPipeline():
    ...
    def shap_visualization(self,x_test,meas,test_index):
        ...
        meas.test_sets.append(test_index)
...
class InitPipeline(ClassifierPipeline):
    ...
    def __init__ (self,args,x_train,y_train,x_test,y_test,meas,test_index):
        ...
        meas = self.shap_visualization(x_test,meas,test_index)                
...
class MultilayerPerceptron_CLF(InitPipeline):
    ...
    def define_classifier(self):        
        clf = MLPClassifier(hidden_layer_sizes=(x_train.shape[1]),max_iter=5000,random_state=self.random_state)
        return(clf)
